import pygame as pg 
from random import randrange

vector = pg.math.Vector2

class Snake:
    def __init__(self, game):
        self.game = game
        self.size = game.TILE_SIZE
        self.rect = pg.rect.Rect([0, 0, game.TILE_SIZE - 2, game.TILE_SIZE - 2])
        self.rect.center = self.get_random_position()
        self.direction = vector(0, 0)
        
        # ? Control the snake size
        self.lenght = 1
        self.segments = []
        
        # ? Add a delay for the snake movement
        self.step_delay = 100 # miliseconds
        self.time = 0
        
        # ? Control movement directions
        self.directions = {pg.K_w: 1, pg.K_a: 1, pg.K_s: 1, pg.K_d: 1}
        
    def control(self, event):
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_w and self.directions[pg.K_w]:
                self.direction = vector(0, -self.size)
                self.directions = {pg.K_w: 1, pg.K_a: 1, pg.K_s: 0, pg.K_d: 1}
            if event.key == pg.K_a and self.directions[pg.K_a]:
                self.direction = vector(-self.size, 0)
                self.directions = {pg.K_w: 1, pg.K_a: 1, pg.K_s: 1, pg.K_d: 0}                
            if event.key == pg.K_s and self.directions[pg.K_s]:
                self.direction = vector(0, self.size)
                self.directions = {pg.K_w: 0, pg.K_a: 1, pg.K_s: 1, pg.K_d: 1}
            if event.key == pg.K_d and self.directions[pg.K_d]:
                self.direction = vector(self.size, 0)
                self.directions = {pg.K_w: 1, pg.K_a: 0, pg.K_s: 1, pg.K_d: 1}
        
    def delta_time(self):
        time_now = pg.time.get_ticks()
        if time_now - self.time > self.step_delay:
            self.time = time_now
            return True
        return False
        
    def get_random_position(self):
        # ? Create snake in random positions 
        return [randrange(self.size // 2, self.game.WINDOW_SIZE - self.size //2, self.size)] * 2
    
    def eats_food(self):
        if self.rect.center == self.game.food.rect.center:
            self.game.food.rect.center = self.get_random_position()
            self.lenght += 1
            self.game.score += 1
            
    
    def check_borders(self):
        if self.rect.left < 0 or self.rect.right > self.game.WINDOW_SIZE:
            self.game.game_over = True 
        if self.rect.top < 0 or self.rect.bottom > self.game.WINDOW_SIZE:
            self.game.game_over = True  
    
    def check_self_eating(self):
        if len(self.segments) != len(set(segment.center for segment in self.segments)):
            self.game.game_over = True  # Game Over
    
    def move(self):
        if self.delta_time():
            self.rect.move_ip(self.direction)
            self.segments.append(self.rect.copy())
            self.segments = self.segments[-self.lenght:]
    
    def update(self):
        self.check_self_eating()
        self.check_borders()
        self.eats_food()
        self.move()
    
    def draw(self):
        [pg.draw.rect(self.game.SCREEN, '#e55381', segment) for segment in self.segments]

class Food:
    def __init__(self, game):
        self.game = game
        self.size = game.TILE_SIZE
        self.rect = pg.rect.Rect([0, 0, game.TILE_SIZE - 2, game.TILE_SIZE - 2])
        self.rect.center = self.game.snake.get_random_position()
        
    
    def draw(self):
        pg.draw.rect(self.game.SCREEN, '#fcf300', self.rect)
        